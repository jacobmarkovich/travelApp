...
document.getElementById("generate").addEventListener("click", () => {
    ...
    if (location && date) { // ensures user enters both a destination and a date
        const inputData = {
            location: location,
            future: fxCall
        }
...
const getData = async (data) => { // server call
...
const checkDate = (input) => { // checks input date, converts it to Unix time which is easier to compare the dates for me.  This part of the project took me a long time to figure out as comparing dates without creating a long loop seemed very difficult
    console.log(input)
    ...
    if (week / 86400 <= 7) {  // checks to see if the trip is less than a week away, then sends the appropriate string to the server for the API call
